...
The following code captures the data cleaning, aggregation, as well as combination of data types required by our modeling. 
...
# Retailer vs non Retailer

As established in exploratory data analysis, there exists a need to solve Swire's problem by splitting customers into two groups, retailers and non retailers. There are both structural and behavioral differences between Retailer and Non retailer customers, retailer customers have features that cannot be applied to non retailers, such as outlet size, geographical spread of outlets, and proportion of well performing outlets. These features can be utilized to further understand retailer customers. Further, as found in our modeling, it is important to analyze retailer customers from the customer level, else there are missed growth opportunities as well as poor performing customers.  
...
Using a decision tree, we found that customers who belong to a retailer group (as indicated by has_outlet) are significantly more likely to be high-volume purchasers. The number of outlets in a retailer group also contributes, although to a lesser degree. The decision tree also confirms the LMP groups to be high ordering individuals. 

### Model 2: Mighty Minis

Investigating the importance of outlet count into the performance of retail customers. Looking at "mini" retailers, those with 3 or less outlets and comparing ordering volume across retailers of this size. We do see that retailers at smaller sizes can still order in large volumes, which indicates that outlet count alone is not a very significant indicator of order volume of a retailer.
...
Using logistic modeling to understand how to determine future order volume for retailer customers, can we determine with statistical significance that past outlet performance (prior order volume). predicts future volume amounts. Before utilizing current order volumes as a feature in understanding Swire's customers we need to evaluate whether or not prior order performance is strongly associated with future order volume- we have seen in early EDA a limited range of features besides order volume that help explain the variability in ordering volume for Swires customers, and are realizing that order volume may need to be the main data point used to explain retailer order behavior. 

We also want to explore if having at least one well-performing outlet in 2023 is associated with a significantly higher likelihood of crossing the 400-gallon threshold in 2024, where well performing is an outlet already ordering above the threshold. 
...
This also helps to further explain the importance of viewing retailers on the customer level rather than a retailer aggregate, as outlet level performance can not be gleaned from the aggregated retailer level. 
...
# new optimal threshold for the model: 0. 4583473
...
for (i in 1:5) {
  ...
  # classify using the custom threshold
...
To validate the strength of our retail customer growth model, we performed 5-fold cross-validation using a custom threshold optimized for high sensitivity. The model consistently achieved an AUC of ~0.97 and identified nearly 97% of growth-ready customers while maintaining high specificity. This gives us confidence in our ability to proactively identify and prioritize future high-performing accounts. This could be a helpful model for Swire to apply to its retailer customers, however we are still viewing customers from an aggregated level and need to work into analysis at the outlet level. 
...
# predict a 2025 over under 400 using the new threshold.
Annual_Customer_Retailer <- Annual_Customer_Retailer %>%
  mutate(
...
```
The model has strong explanatory power, and we wanted to utilize it to generate simulated order predictions for 2025. 
...
Annual_Customer_Retailer <- Annual_Customer_Retailer |>
  mutate(
    risk_reward_segment = case_when(
      ...
      Binning_column == "transtionary growing" ~ "Emerging Opportunity",  # had issues here, realized theres a space on the bin name
      Binning_column == "transitionary declining" ~ "Intervention Needed",
      TRUE ~ "Unclassified"
    )
  )

# now i want to filter out the reliable customers, for those with at least 1000 gallons ordered in both years. 
...
AUc decreased to 91.8 when removing the reliable performers from the analysis. 


now just predicting 2025 for this "non reliable" group

```{r}
# predict 2025 for this segment, to see if the removed noise from the super high orders helps us in our understandings 
...
57 individual retailers, but about 450 individual customers within those retailers, transitional growing segments had the largest amount of customers anticipated to be above threshold in 2025. 

We tested two approaches to forecasting 2025 volume, A logistic regression model trained on 2023 and 2024 performance and outlet data, and a rule-based YoY simulation with noise to mimic real-world volatility.The results aligned 99.4 percent of the time, which gave us confidence in the predictive value of our model. 
...
### Model 4: Segment analysis

Reliable Performers and Emerging Opportunities have significantly higher average volume per outlet than other groups, and justify direct routing investment (red truck)
...
### Model 5: Multi level models

start with brief reformatting of the data to allow for individual customer level analysis within retail groups
...
Utilizing multi level modeling to understand the impact of specif retail groups, LMP status, Co2 use, as well as geographical clusters. We have reformatted the data to explore the impact of retailer ordering behavior on the outlet level (customers part of a parent group).
...
retailer_set <- retailer_set |>
  mutate(
    ON_BOARDING_DATE = mdy(ON_BOARDING_DATE),         
    onboarding_year = year(ON_BOARDING_DATE),           
    customer_age = 2025 - onboarding_year)
...
# remove customers from the modeling who are the only ones in their retail groups. 
retailer_set <- retailer_set |>
...
```
Starting with an initial mlm using retailer groups as a random effect, this is to see if there is significant unexplained effects on the ordering behavior of the customers a part of specific retailers. 
...
```
this secondary multi level model used the customer segments as a random effect, this would help us determine if there is explanatory power in the customer segments. We found that customer segments can explain meaningful variation, even after controlling fro known predictors (Co2 ordering, order type, customer age, and percent change YoY), so we can better predict a customers performance if we know their segment. It also helps us to determine that these segments alone hold explanatory power in customers ordering behavior. 
...
```
This final mlm model was a combination of retailer group and customer segments. Using this model we were able to explain the most variance when group-level patterns are considered.
...
Cross validated performance is strong. 
...
# filter for new growers, those not yet over 400 in 2024
...
```
The mlm provides strong explanatory power when applied to Swires current data, however the mlm required the use of the two years of data, in order to generate business impact through simulated 2025 predictions for the presentation there is a concern of target leakage. However our belief is that this modeling process is strongest in its explanatory power of highlighting customers within segments that are poised to grow beyond threshold, and should be flagged to ensure they are not moved to an alternate route to market. 

We had also explored multi level models using cluster as the random effects, but the model has singular fit, meaning as a random effect location doesn't explain much in customer ordering behavior. 
